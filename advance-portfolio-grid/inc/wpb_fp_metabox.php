...
function wpb_fp_add_portfolio_meta_boxes()
{
  ...
  add_meta_box(
    ...
    esc_html__('Portfolio External link', 'advance-portfolio-grid'),    // Title
...
{ ?>
  ...
  <p>
    <label for="wpb-fp-portfolio-url"><?php echo esc_html__("Portfolio external link, If not provided it will linking to single portfolio.", 'advance-portfolio-grid'); ?></label>
  </p>
  <p>
...
function wpb_fp_save_portfolio_class_meta($post_id, $post)
{
  ...
  if (!isset($_POST['wpb_fp_portfolio_ex_link_nonce']) || !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['wpb_fp_portfolio_ex_link_nonce'])), basename(__FILE__)))
    return $post_id;
  ...
  $new_meta_value = isset($_POST['wpb-fp-portfolio-url']) ? sanitize_text_field(wp_unslash($_POST['wpb-fp-portfolio-url'])) : '';
...
function wpb_fp_add_portfolio_meta_boxes_pro_add()
{
  ...
  add_meta_box(
    ...
    esc_html__('Pro version features', 'advance-portfolio-grid'),
