...
function wpb_fp_shortcode_funcation($atts)
{
	...
	if (isset($_GET['wpb_hover_effect'])) {
		$hover_effect = sanitize_text_field(wp_unslash($_GET['wpb_hover_effect'])); // phpcs:ignore WordPress.Security.NonceVerification.Recommended
	}
	...
	if ($loop->have_posts()) {
		...
		if ($type == 'slider' && $enable_slider != 'on') {
			$output .= '<div class="wpb_fp_notice wpb_fp_notice_error">' . esc_html__('Please, enable the slider from the settings.', 'advance-portfolio-grid') . '</div>';
		}
		...
		while ($loop->have_posts()) : $loop->the_post();
			...
			if (isset($wpb_fp_show_overlay) && $wpb_fp_show_overlay == 'show'):
				...
				if (isset($wpb_fp_show_links) && $wpb_fp_show_links == 'show'):
					$output .= '<p class="wpb_fp_icons wpb_fp_icons_has_ex_link_' . esc_attr($wpb_fp_portfolio_ex_link ? 'yes' : 'no') . '">';
					...
					if ($show_details_btn == 'on') {
						if ($wpb_fp_portfolio_ex_link && $wpb_fp_portfolio_ex_link != '') {
				...
				if ($wpb_fp_full_grid_link == 'yes') {
					if ($wpb_fp_link_full_grid_type == 'details_page') {
						if ($wpb_fp_portfolio_ex_link && $wpb_fp_portfolio_ex_link != '') {
							$output .= '<a class="wpb_fp_full_grid_link" href="' . esc_url($wpb_fp_portfolio_ex_link) . '"></a>';
						} else {
							$output .= '<a class="wpb_fp_full_grid_link" href="' . esc_url(get_the_permalink()) . '"></a>';
						}
					} else {
			...
			if (isset($wpb_fp_portfolio_ex_link) && !empty($wpb_fp_portfolio_ex_link)) {
				$output .= '<a class="wpb_fp_btn" href="' . esc_url($wpb_fp_portfolio_ex_link) . '" target="_blank">' . esc_html(wpb_fp_get_option('wpb_fp_view_portfolio_btn_text_', 'wpb_fp_advanced', esc_html__('View Portfolio', 'advance-portfolio-grid'))) . '</a>';
			}
			$output .= '</div>';
			$output .= '</div>';
			$output .= '</div>';
		...
		$output = esc_html__('No portfolio found', 'advance-portfolio-grid');
	}
	wp_reset_postdata();
	return $output;
}
